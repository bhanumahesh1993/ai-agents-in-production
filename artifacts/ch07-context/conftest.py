...
``pytest`` run over all of ``artifacts/`` that has a consequence: Chapter 7
and Chapter 11 both ship a ``budget.py``, Chapter 2 ships a third, and every
chapter ships a ``demo.py``, so whichever chapter's tests ran first would own
the plain module name for the rest of the session.
